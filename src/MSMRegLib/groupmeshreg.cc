...
void GroupMeshReg::Initialize_level(int level)
{
    ...
    if (cost[level] == "AFFINE") {
        ...
        if (multivariate == true)
            throw MeshReg_error("GroupMeshREG ERROR:: multivariate not currently available");
...
void GroupMeshReg::Evaluate()
{
    ...
    for (int n = 0; n < MESHES.size(); n++) {
        ...
        if (_level == 1)
            ALL_SPH_REG.push_back(project_CPgrid(SPH_orig, OLDREG)); // first project data grid through any predefined transformation or, from transformation from previous resolution level
        else {
...
void GroupMeshReg::saveTransformedData(const double& sigma, const string& filename)
{
    ...
    double ang;
    ang = R.guess_angular_spacing(TEMPLATE.nvertices());
    char fullpath[1000];
...
void GroupMeshReg::run_discrete_opt(vector<NEWMESH::newmesh>& source)
{
    ...
    it = PARAMETERS.find("CPres");
    res = boost::get<int>(it->second);
    ...
    while (iter <= _itersforlevel) {
        ...
        if (iter > 1 && ((iter - 1) % 2 == 0) && newenergy >= energy) {
            cout << iter << " level has converged" << (iter - 1) % 2 << endl;
            break;
        }
        ...
        for (int n = 0; n < source.size(); n++) {
            ...
            cout << n << " transformed_controlgrid.nvertices() " << transformed_controlgrid.nvertices() << " controlgrid[n].nvertices() " << controlgrid[n].nvertices() << " source vertices "
                 << " " << source[n].nvertices() << " " << controlgrid_neighbourhood->Ncols() << endl;
