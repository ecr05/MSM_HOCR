...
void MeshReg::run_multiresolutions(const int& levels, const double& sigma, const string& parameters)
{
    ...
    if (levels != 0)
        _resolutionlevels = levels;

    for (int i = 0; i < _resolutionlevels; i++) {
        _level = i + 1;
        if (_verbose)
            cout << " Initialising level " << i + 1 << endl;
        ///// set parameters /////////////////
        fix_parameters_for_level(i);
        Initialize_level(i);
        Evaluate();
        if (cost[i] == "AFFINE")
            affinecf.reset(); // clear memory following affine initilisalisation
    }
...
void MeshReg::Initialize_level(int level)
{
    ...
    sigma[0] = _sigma_in[level];
    sigma[1] = _sigma_ref[level];
    ...
    if (cost[level] == "AFFINE") { /// currently the affine alignment uses a different class to the discrete, but will hopefully implement an affine discrete optimisation soon
        ...
        if (_simval[level] != 1)
            affinecf->set_simmeasure(1);
        affinecf->Initialize();
        _isaffine = true;
    }
    if (cost[level] == "DISCRETE") {
        if (_simval[level] == 3)
            cout << " warning NMI similarity measure does not take into account cost function weights or exclusion values" << endl;
        bool multivariate;
        if (FEAT->get_dim() == 1) {
            multivariate = false;
            if (_simval[level] == 4) {
                throw MeshReg_error("MeshREG ERROR:: simval option 4 (alphaMI) is not suitable for univariate costfunctions");
            }
        } else
            multivariate = true;
        ...
        CONTROL.make_mesh_from_icosa(_gridres[level]);
        true_rescale(CONTROL, RAD);
        ...
        if (_anat) {
            ...
            if (_regmode == 5) {
                throw MeshReg_error("STRAINS based regularisation requires anatomical meshes");
            } else if (_regmode == 4) {
                throw MeshReg_error("You have specified angular based penalisation of anatomical warp, for which you require anatomical meshes");
            }
        }
...
newmesh MeshReg::resample_anatomy(newmesh control_grid, vector<map<int, double>>& baryweights, vector<vector<int>>& ANAT_to_CPgrid_neighbours, int level)
{
    ...
    if (in_anat.nvertices() != MESHES[0].nvertices() || ref_anat.nvertices() != MESHES[1].nvertices()) {
        throw MeshReg_error("MeshREG ERROR:: input/reference anatomical mesh resolution is inconconsistent with input/reference spherical mesh resolution ");
    }
    ...
    if ((_anatres[level] - _gridres[level]) > 0) {
        ...
        if (ANAT_to_CPgrid_neighbours.size() == 0) {
    ...
    for (unsigned int i = 0; i < ANAT_to_CPgrid_neighbours.size(); i++) {
        id0 = control_grid.get_triangle_vertexID(i, 0);
        id1 = control_grid.get_triangle_vertexID(i, 1);
        id2 = control_grid.get_triangle_vertexID(i, 2);
        v0 = control_grid.get_triangle_vertex(i, 0);
        v1 = control_grid.get_triangle_vertex(i, 1);
        v2 = control_grid.get_triangle_vertex(i, 2);
...
{
    if (_verbose)
        cout << " save transformed data " << endl;
    char fullpath[1000];
    ...
    if (sigma < 1e-3) {
        ...
        if (MESHES[1].nvertices() > MESHES[0].nvertices()) {
            ang = R.guess_angular_spacing(MESHES[0].nvertices());
        } else {
            ang = R.guess_angular_spacing(MESHES[0].nvertices());
        }
    } else {
        R.set_method("GAUSSIAN");
        ang = 2 * asin(sigma / (RAD));
    }
    ...
    if (_anat) {
        ...
        boost::shared_ptr<Matrix> VECS;
        VECS = boost::shared_ptr<Matrix>(new Matrix);
        in_anat.estimate_normals();
        ANAT_TRANS.estimate_normals();
        newmesh STRAINSmesh = calculate_strains(2, in_anat, ANAT_TRANS, VECS);
        calculate_triangular_strains(in_anat, ANAT_TRANS);
...
NEWMESH::newmesh MeshReg::project_CPgrid(NEWMESH::newmesh SPH_in, NEWMESH::newmesh REG, int num)
{ /// num indices which warp for group registration
    ...
    if (_level == 1) {
        ...
        icotmp.make_mesh_from_icosa(ico);
        ...
        if (MODEL.get())
            MODEL->warp_CPgrid(inorig, incurrent);
...
void MeshReg::run_discrete_opt(NEWMESH::newmesh& source)
{
    ...
    it = PARAMETERS.find("iters");
    _itersforlevel = boost::get<int>(it->second);
    ...
    while (iter <= _itersforlevel) {
        ...
        if (_discreteOPT == "FastPD") {
            ...
            if (_discreteOPT.compare(0, 4, "HOCR") == 0)
                ...
                throw MeshReg_error("discrete optimisation mode is not available");
            }
        ...
        if (iter > 1 && ((iter - 1) % 2 == 0) && (energy - newenergy < 0.001)) {
            if (_verbose)
                cout << iter << " level has converged" << (iter - 1) % 2 << " newenergy " << newenergy << " energy " << energy << " energy- newenergy " << energy - newenergy << endl;
            break;
        } else {
            if (_verbose)
                cout << " newenergy " << newenergy << " energy " << energy << " energy- newenergy " << energy - newenergy << endl;
        }
        ...
        if (_verbose)
            cout << "Output Control Point Label assignment:" << endl;
