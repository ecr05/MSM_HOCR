...
int main(int argc, char** argv)
{
    ...
    for (int it = 0; it < resolutionlevels; it++) {
        ...
        ico.make_mesh_from_icosa(anat_res[it]);
        true_rescale(ico, RAD);
        SPHEREres = ico;
        ANATres = mesh_resample(ANAT, SPHERE, SPHEREres);
        STRAINS = ANATres;

        if (it == (resolutionlevels - 1)) {
            sprintf(filename, "%sSPHEREINIT.LR.surf", outdir.c_str());
            SPHEREres.save(filename);
            sprintf(filename, "%sANAT.LR.surf", outdir.c_str());
            ANATres.save(filename);
        }
        ...
        if (it > 0) {
            MODEL->warp_CPgrid(controlgrid, transformed_controlgrid);
            SPHEREres = MODEL->get_CPgrid();
        }
        ...
        while (iter < iters_res[it]) {
            ...
            MODEL->applyLabeling();
            if (iter > 0 && iter % 2 == 0 && newenergy >= energy) {
                cout << iter << "energy" << energy << " newenergy " << newenergy << " level has converged" << (iter - 1) % 2 << endl;
                break;
            }
