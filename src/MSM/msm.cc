...
int main(int argc, char* argv[])
{
    ...
    if (opts.verbose.value())
        MR.set_verbosity(opts.verbose.value());
    if (opts.debug.value())
        MR.set_debug(opts.debug.value());

    MR.set_input(opts.inputmesh.value());
    if (opts.referencemesh.value() == "")
        MR.set_reference(opts.inputmesh.value());
    else
        MR.set_reference(opts.referencemesh.value());

    ////// add anatomical meshes for stress and strain ////////////
    if (opts.inputanatmesh.value() != "") {
        if (opts.referenceanatmesh.value() == "") {
            cout << " Error: must supply both anatomical meshes or none " << endl;
            exit(1);
        }
    ...
    if (opts.transformed_sphere.set())
        MR.set_transformed(opts.transformed_sphere.value());
    if (opts.cfweight_in.set())
        MR.set_input_cfweighting(opts.cfweight_in.value());
    if (opts.cfweight_ref.set())
        MR.set_reference_cfweighting(opts.cfweight_ref.value());
    ...
    if (opts.in_register.set()) {
        ...
        char filename[1000];
        if (opts.transformed_sphere.set())
            target.load(opts.transformed_sphere.value());
        else
            target.load(opts.inputmesh.value());
    ...
    if (opts.in_register.set()) {
        ...
        if (opts.transformed_sphere.set())
            ORIG.load(opts.transformed_sphere.value());
        else
            ORIG.load(opts.inputmesh.value());
