...
int main(int argc, char** argv)
{
    ...
    while (argc > 1) {
        ...
        if ((ok == 0) && (strcmp(argv[1], "-target") == 0)) {
            ...
            if (INsurflist.size() != INlist.size()) {
                cout << " surf list is not the same length as data list" << endl;
                exit(1);
            }
            argc--;
            argv++;
    ...
    exclusion.ReSize(TARG.nvertices());
    exclusion = 1;
    ...
    for (unsigned int i = 0; i < INlist.size(); i++) {
        ...
        if (dim != atlasdim) {
            cout << i << " dim " << dim << " atlasdim " << endl;
            throw MeshReg_error("Feature dimensions do not agree");
        }

        if (_weighted) {
            if (_sigma == 0) {
                cout << "must set sigma for weighted" << endl;
                exit(1);
            }
        ...
        for (int d = 0; d < atlasdim; d++) {
            ...
            for (int j = 0; j < AVERAGE.nvertices(); j++) {
                if (EXCL_ref->get_pvalue(j) && abs(IN.get_pvalue(j, d)) > EPSILON) {
                    if (_abs)
                        newval = AVERAGE.get_pvalue(j, d) + weight[i] * abs(IN.get_pvalue(j, d));
                    else
                        newval = AVERAGE.get_pvalue(j, d) + weight[i] * IN.get_pvalue(j, d);
    ...
    for (int d = 0; d < atlasdim; d++) {
        for (int i = 0; i < AVERAGE.nvertices(); i++) {
            if (totalcounts.get_pvalue(i, d) == 0)
                newval = 0;
            else
                newval = AVERAGE.get_pvalue(i, d) / (totalcounts.get_pvalue(i, d));
            AVERAGE.set_pvalue(i, newval, d);
            if (_avdim == -1 || _avdim == d)
                sumav += newval;
        ...
        for (int d = 0; d < atlasdim; d++) {
            for (int j = 0; j < AVERAGE.nvertices(); j++) {
                if (EXCL_ref->get_pvalue(j) && exclusion(j + 1)) {
                    if (_abs)
                        newval = weight[i] * (abs(INPUTS[i].get_pvalue(j, d)) - AVERAGE.get_pvalue(j, d));
                    else
                        newval = weight[i] * (INPUTS[i].get_pvalue(j, d) - AVERAGE.get_pvalue(j, d));
