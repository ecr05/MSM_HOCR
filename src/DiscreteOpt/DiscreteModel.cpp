...
{
    if (labeling)
        std::fill(labeling, labeling + m_num_nodes, 0);
}
...
void SRegDiscreteModel::set_parameters(myparam PAR)
{
    ...
    it = PAR.find("lambda");
    m_lambda = boost::get<float>(it->second);
    it = PAR.find("CPres");
    m_CPres = boost::get<int>(it->second);
    it = PAR.find("SGres");
    m_SGres = boost::get<int>(it->second);
    it = PAR.find("simmeasure");
    m_simmeasure = boost::get<int>(it->second);
    it = PAR.find("regularisermode");
    m_regoption = boost::get<int>(it->second);
    it = PAR.find("multivariate");
    m_multivariate = boost::get<bool>(it->second);
    it = PAR.find("verbosity");
    m_verbosity = boost::get<bool>(it->second);
    it = PAR.find("outdir");
    m_outdir = boost::get<string>(it->second);
    it = PAR.find("TriLikelihood");
    m_triclique = boost::get<bool>(it->second);
    it = PAR.find("rescalelabels");
    m_rescalelabels = boost::get<bool>(it->second);
    it = PAR.find("numthreads");
    _numthreads = boost::get<int>(it->second);
    it = PAR.find("quartet");
    _estquartet = boost::get<bool>(it->second);
...
void SRegDiscreteModel::Initialize(const newmesh& CONTROLGRID)
{
    ...
    if (costfct.get()) {
        ...
        throw DISCRETEOPTHOCRException("Discrete Model:: You have not initialised the discrete costfunction before initialising the model");
    }
...
void SRegDiscreteModel::initialize_cost_function(const bool& MV, const int& sim, myparam& P)
{
    ...
    if (MV) {
        ...
        throw DISCRETEOPTHOCRException("DiscreteModel ERROR:: alpha MI not in this version of MSM yet");
    } //costfct=boost::shared_ptr<SRegDiscreteCostFunction>(new alphaMINonLinearSRegDiscreteCostFunction());
    else {
        if (m_triclique)
...
void SRegDiscreteModel::Initialize_sampling_grid()
{
    ...
    m_samplinggrid.make_mesh_from_icosa(m_SGres);
            ...
            for (vector<int>::const_iterator j = Grid.tIDbegin(getneighbours[i]); j != Grid.tIDend(getneighbours[i]); j++) {
                ...
                if ((bary - centre).norm() <= dist && (bary - centre).norm() > 0 && found_tr(*j + 1) == 0) {
                    for (unsigned int k = 0; k < m_barycentres.size(); k++) {
                        if (abs(1 - (((bary - centre) | (m_barycentres[k] - centre)) / ((bary - centre).norm() * (m_barycentres[k] - centre).norm()))) < 1e-2) {
                            found_tr(*j + 1) = 1;
                        }
                    }
                    if (!found_tr(*j + 1)) {
                        m_barycentres.push_back(bary);
                        Grid.set_pvalue(Grid.get_triangle_vertexID(*j, 0), label);
                        label++;
                    }
                    found_tr(*j + 1) = 1;
                }
...
vector<Pt> SRegDiscreteModel::rescale_sampling_grid()
{
    ...
    Pt sample, newsample;
    if (m_verbosity)
        cout << " resample labels " << m_scale << " length scale " << (centre - m_samples[1]).norm() << endl;
...
void NonLinearSRegDiscreteModel::get_rotations(vector<Matrix>& ROT)
...
void NonLinearSRegDiscreteModel::setupCostFunction()
{
    ...
    if (m_debug) {
        char filename[1000];
        sprintf(filename, "%sSOURCE-%d.surf", m_outdir.c_str(), m_iter);
        m_SOURCE.save(filename);
        sprintf(filename, "%sSOURCE-%d.func", m_outdir.c_str(), m_iter);
        m_SOURCE.save(filename);

        if (m_iter == 1) {
            sprintf(filename, "%sTARGET.surf", m_outdir.c_str());
            m_TARGET.save(filename);
        }
        sprintf(filename, "%sCPgrid-%d.surf", m_outdir.c_str(), m_iter);
    ...
    if (m_verbosity)
        cout << " initialize cost function " << m_iter << endl;
