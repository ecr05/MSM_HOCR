...
public:
    ...
    virtual void set_meshes(const NEWMESH::newmesh& target, const NEWMESH::newmesh& source, const NEWMESH::newmesh& GRID, int num = 1)
    {
        _TARGET = target;
        _SOURCE = source;
        _ORIG = source;
    ...
    void set_meshes(const NEWMESH::newmesh& target, const NEWMESH::newmesh& source)
    {
        _TARGET = target;
        _SOURCE = source;
    };

    void set_anatomical(const NEWMESH::newmesh& targetS, const NEWMESH::newmesh& targetA, const NEWMESH::newmesh& sourceS, const NEWMESH::newmesh& sourceA)
    {
        _TARGEThi = targetS;
        _aTARGET = targetA;
        _aICO = sourceS, _aSOURCE = sourceA;
    }
    void set_anatomical_neighbourhood(const vector<map<int, double>>& weights, const vector<vector<int>> neighbourhood)
    {
        _ANATbaryweights = weights;
        NEARESTFACES = neighbourhood;
    }
    ...
    virtual void set_relations(const boost::shared_ptr<RELATIONS>& CONTROL, const boost::shared_ptr<RELATIONS>& TARG)
    {
        _controlrel = CONTROL;
        _targetrel = TARG;
    ...
    virtual void set_spacings(const ColumnVector& spacings, const double MAX, int num = 0)
    {
        MAXSEP = spacings;
        MVDmax = MAX;
    }
    ...
    void set_labels(const vector<Pt> labellist, const vector<Matrix> ROT = vector<Matrix>())
    {
        _labels = labellist;
        ROTATIONS = boost::shared_ptr<vector<Matrix>>(new vector<Matrix>(ROT));
    } // ROTATION MATRIX MAY ONLY BE REQUIRED FOR NON LINEAR FRAMEWORK
    ...
    {
        double dist = 0.0;
        if (within_controlpt_range(CPindex, sourceindex, dist))
            return true;
        else
            return false;
    }
    ...
    bool within_controlpt_range(const int& CPindex, const int& sourceindex, double& dist)
    {
        ...
        if (dist < _controlptrange * MAXSEP(CPindex + 1))
            return true;
        else
            return false;
    }
    ...
    {
        if (_debug)
            cout << " sumlikelihood " << sumlikelihood << " sumregcost " << sumregcost << endl;
    }
...
public:
    ...
    void reset_target_data(int node)
    {
        for (unsigned int i = 0; i < _sourceinrange[node - 1].size(); i++) {
            _targetdata[_sourceinrange[node - 1][i] - 1].clear();
        }
    } /// *****NOT THREAD SAFE ********

    virtual void resample_target_data(const Pt&, const Pt&, const Pt&, const Pt&, const int&, const int&, const int&, int&);
    void get_data_index(const int& controlpoint, const int& sourcepoint, int& ind)
    {
        if (sourcepoint > (int)_sourcedata.size()) {
            cout << "vector index too high " << endl;
            exit(1);
        }
        ind = sourcepoint;
    }
...
public:
    ...
    // Constructor.
    //
    MetricDistortionDiscreteCostFunction(double shear, double bulk, double exp)
        : RegularisationDiscreteCostFunction(shear, bulk, exp)
    {
        isinitialised = false;
    }
