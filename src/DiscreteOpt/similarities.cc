...
void simkernel::set_input(string path_to_input)
{ // assume matrix is full??
...
void simkernel::Initialize(int simval)
{
    ...
    if (m_A == NULL) {
        throw DISCRETEOPTHOCRException("SIMILARITIES:: Connectivity matrices have not been initliased");
    }
...
void simkernel::Initialize(int simval, const vector<double>& inputdata, const vector<double>& refdata, const vector<double>& weights)
{
    ...
    if (_sim == 3) {
        ...
        for (unsigned int i = 0; i < inputdata.size(); i++) {
            ...
            sum += varwght;
            if (_meanA != _meanA)
                cout << i << "weights.size " << weights.size() << " weights[i] " << weights[i] << " weight " << varwght << " inputdata[i] " << inputdata[i] << endl;
            if (_meanB != _meanB)
                cout << i << "weights.size " << weights.size() << " weights[i] " << weights[i] << " weight " << varwght << " refdata[i] " << refdata[i] << endl;
        }
        if (sum > 0) {
            _meanA /= sum;
            _meanB /= sum;
        ...
        if (_meanA != _meanA)
            cout << _meanA << " mean A nan " << sum << endl;
        if (_meanB != _meanB)
            cout << _meanB << " mean B nan " << sum << endl;
    }
}
...
double simkernel::corr(const vector<double>& A, const vector<double>& B, const vector<double>& weights)
{
    ...
    if (A.size() != B.size()) {
        ...
        throw DISCRETEOPTHOCRException("SIMILARITIES:: correlation, data dimensions do not match");
    }
    ...
    for (unsigned int s = 0; s < A.size(); s++) {
        ...
        sum += varwght;
        if (sum != sum)
            cout << " varwght " << varwght << endl;
        if (prod != prod)
            cout << s << " prod " << prod << " A[s] " << A[s] << " " << B[s] << "_meanA " << _meanA << " _meanB " << _meanB << endl;
        if (varA != varA)
            cout << s << " varA " << varA << " A[s] " << A[s] << " " << B[s] << "_meanA " << _meanA << " _meanB " << _meanB << endl;
        if (varB != varB)
            cout << s << " varB " << varB << " A[s] " << A[s] << " " << B[s] << "_meanA " << _meanA << " _meanB " << _meanB << endl;
    }
...
double simkernel::corr(const map<int, float>& A, const map<int, float>& B)
{
    ...
    if (ind == 0) {
        cout << " ind==0 << A.size() " << A.size() << " " << B.size() << endl;
        val = MAXSIM;
    } else {
        ...
        if (varA == 0.0 || varB == 0.0)
            ...
            val = prod / (sqrt(varA) * sqrt(varB));

            val = 1 - (1 + val) / 2;
        }
        if (val != val)
            cout << "prod " << prod << " varA " << varA << " varB " << varB << " ind " << ind << endl;
    }

    if (val != val)
        cout << " 2 prod " << prod << " varA " << varA << " varB " << varB << " ind " << ind << endl;
...
double simkernel::SSD(const vector<double>& A, const vector<double>& B, const vector<double>& weights)
{
    ...
    if (A.size() != B.size()) {
        throw DISCRETEOPTHOCRException("SIMILARITIES:: SSD data dimensions do not match");
    }
...
double simkernel::nSSD(const vector<double>& A, const vector<double>& B, const vector<double>& weights)
{ /// normalized by variance - was hoped this would make SSD better for discrete opt but doesn't seem to be the case
    ...
    if ((sqrt(varA) * sqrt(varB)) > 0)
        prod = prod / (sqrt(varA) * sqrt(varB));
...
double simkernel::NMI(const vector<double>& A, const vector<double>& B, const vector<double>& weights)
{
    double val;
    if (A.size() != B.size()) {
        throw DISCRETEOPTHOCRException("SIMILARITIES:: NMI data dimensions do not match");
    }
    ...
    for (unsigned int s = 0; s < A.size(); s++) {
        double varwght = 0;
        if (weights.size() == A.size())
            varwght = 1; //weights[s];  // Haven't verified weighted NMI yet
        else
            varwght = 1;
