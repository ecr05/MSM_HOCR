...
void GroupDiscreteCostFunction::set_parameters(myparam& ALLPARAMS)
{
    ...
    it = ALLPARAMS.find("lambda_pairs");
    _lambdapairs = boost::get<float>(it->second);
    it = ALLPARAMS.find("set_lambda_pairs");
    _setpairs = boost::get<bool>(it->second);
    ...
    it = ALLPARAMS.find("quartet");
    _quadcost = boost::get<bool>(it->second);
...
void GroupDiscreteCostFunction::initialize(int numNodes, int numLabels, int numPairs, int numTriplets, int numQuartets)
{
    ...
    if (!_targetrel.get()) {
        throw DISCRETEOPTHOCRException(" GroupDiscreteCostFunction::initialize  must initialise RELATIONS in MODEL before running this function");
    }
    if (_CONTROLMESHES.size() == 0) {
        throw DISCRETEOPTHOCRException(" GroupDiscreteCostFunction::initialize  must initialise  meshes before running this function");
    }
    ...
    //   L1data=0;
    if (_quadcost == true && _setpairs == false) {
        _lambdapairs = (float)m_num_quartets / (float)m_num_pairs;
        cout << m_num_quartets << " true " << m_num_pairs << " " << (float)m_num_quartets / (float)m_num_pairs << endl;
    }
    cout << " INIT " << _reglambda << " _lambdapairs " << _lambdapairs << " setpair " << _setpairs << endl;
}

void GroupDiscreteCostFunction::define_template_patches()
{ /// make patches cover overlap with all between subject neighbours
    int ind, mesh_ID, num;
    ...
    if (!_pairs) {
        throw DISCRETEOPTHOCRException(" GroupDiscreteCostFunction::initialize  must set pairs before function");
    }
...
double GroupDiscreteCostFunction::computeQuartetCost(int quartet, int labelA, int labelB, int labelC, int labelD)
{
    ...
    Matrix Groupdata(rows.size(), num_subjects);
    Groupdata = 0;
    // L1data = new double[num_subjects];
...
void GroupDiscreteCostFunction::resampler_worker_function(const int& begin, const int& end, const vector<bool>& INrange)
{
    ...
    for (int n = begin; n < end; n++) {
        ...
        if (INrange[labnum + num * _labels.size()] == true) {
            ...
            cout << " (newpt-CP).norm() > 0.5*SPACINGS[meshID](node+1) " << endl;
        }
        if (n % 1000 == 0)
            cout << " resample patch: " << n << " " << labnum << " " << PATCHDATA[labnum + num * _labels.size()].size() << endl;
    }
}

map<int, float> GroupDiscreteCostFunction::resample_onto_template(const int& node, const int& ID, const Pt& newCP, const vector<int>& PTS)
{ //, const RELATIONS
    ...
    for (vector<int>::const_iterator j = _CONTROLMESHES[ID].tIDbegin(node); j != _CONTROLMESHES[ID].tIDend(node); j++) {
        ...
        CP0 = tri.get_vertex_coord(0);
        CP1 = tri.get_vertex_coord(1);
        CP2 = tri.get_vertex_coord(2);
        id0 = tri.get_vertex_no(0);
        id1 = tri.get_vertex_no(1);
        id2 = tri.get_vertex_no(2);
        ...
        if (id0 == node) {
            ...
            cout << node << " triangle node not found "
                 << " id0" << id0 << " " << id1 << " " << id2 << endl;
            exit(1);
        }
